    ...
    void* data, const size_t length)
{
    if(length > USHRT_MAX)
    {
        GPIB_DPRINTK("length (%zu) too big (max = %d)", length, USHRT_MAX);
        return -EINVAL;
    }
    ...
    const size_t len)
{
    if(verbose)
    {
        GPIB_DPRINTK("%@, addr 0x%04x len %4zd (0x%04zx)", label, addr, len, len);
    }
    ...
    const BOOL doRun)
{
    ...
    if(verbose)
    {
        GPIB_DPRINTK("%@", data ? @"stop CPU" : @"reset CPU");
    }
    ...
    if(status != 1)
    {
        GPIB_DPRINTK("Can't modify CPUCS");
        return FALSE;
    }
    ...
    const part_type partType, const BOOL stage)
{
    ...
    if (image == 0)
    {
        GPIB_DPRINTK("%@: unable to open for input.", hexfilePath);
        return -2;
    }
    else if (verbose)
    {
        GPIB_DPRINTK("open RAM hexfile image %@", hexfilePath);        
    }
    ...
    else
    {
        ...
        if (verbose)
        {
            GPIB_DPRINTK("2nd stage:  write external memory");
        }
    }
    ...
    if (status < 0)
    {
        GPIB_DPRINTK("unable to download %@", hexfilePath);
        return status;
    }
    ...
    if (stage == TRUE)
    {
        ...
        if (verbose)
        {
            GPIB_DPRINTK("2nd stage:  write on-chip memory");            
        }
            
        status = parse_ihex (image, &ctx, is_external, ram_poke);
        if (status < 0)
        {
            GPIB_DPRINTK("unable to completely download %@", hexfilePath);
            return status;
        }
    }
    ...
    if (verbose)
    {
        GPIB_DPRINTK("... WROTE: %d bytes, %d segments, avg %d",
    ...
    const part_type partType, uint8_t config)
{
    ...
    {
        GPIB_DPRINTK("don't see a large enough EEPROM");
        return -1;
    }
    ...
    {
        GPIB_DPRINTK("open EEPROM hexfile image %@", hexfilePath);        
    }

    if (verbose)
    {
        GPIB_DPRINTK("2nd stage:  write boot EEPROM");        
    }
    ...
    else if(partType == ptAN21)
    {
        ...
        ctx.ee_addr = 7;
        config = 0;
        GPIB_DPRINTK("AN21xx:  no EEPROM config byte");
    }
    else
    {
        GPIB_DPRINTK("?? Unrecognized microcontroller type %u ??", partType);
        return -1;
    }
    ...
    if (status < 0)
    {
        GPIB_DPRINTK("unable to write EEPROM %@", hexfilePath);
        return status;
    }
    ...
    if (status < 0)
    {
        GPIB_DPRINTK("unable to append reset to EEPROM %@", hexfilePath);
        return status;
    }
...
)
{
    ...
    for (;;) {
        ...
        if (cp == 0) {
            GPIB_DPRINTK("EOF without EOF record!");
            break;
        }
        ...
        if (buf[0] != ':') {
            GPIB_DPRINTK("not an ihex record: %s", buf);
            return -2;
        }
   ...
   /* Initialize data_addr */
    ...
    if (type == 1) {
        if (verbose >= 2)
        GPIB_DPRINTK("EOF on hexfile");
        break;
    }

    if (type != 0) {
        GPIB_DPRINTK("unsupported record type: %u", type);
        return -3;
    }

    if ((len * 2) + 11 > strlen(buf)) {
        GPIB_DPRINTK("record too short?");
        return -4;
    }
    ...
    const uint8_t *data, const size_t len)
{
     ...
     switch (ctx->mode)
     {
         case internal_only:     /* CPU should be stopped */
         {
             if (external)
             {
                 GPIB_DPRINTK("can't write %zd bytes external memory at 0x%04x", len, addr);
                 return -EINVAL;
             }
         ...
         case skip_internal:     /* CPU must be running */
         {
             if (!external)
             {
                 if (verbose >= 2)
                 {
                     GPIB_DPRINTK("SKIP on-chip RAM, %zd bytes at 0x%04x", len, addr);
                 }
         ...
         case skip_external:     /* CPU should be stopped */
         {
             if (external)
             {
                  if (verbose >= 2)
                  {
                      GPIB_DPRINTK("SKIP external RAM, %zd bytes at 0x%04x", len, addr);
                  }
    ...
    const uint8_t* data, const size_t len)
{
    ...
    if (external)
    {
        GPIB_DPRINTK("EEPROM can't init %zd bytes external memory at 0x%04x", len, addr);
        return -EINVAL;
    }

    if (len > 1023)
    {
        GPIB_DPRINTK("not fragmenting %zd bytes", len);
        return -EDOM;
    }
